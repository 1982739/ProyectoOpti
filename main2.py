...
from parametros import cargar_parametros2
model = Model()

### Cargar parámetros ###
P, Z, Q, T, T2, B, B_pq, γ_zq, d_zp, v_p, C_z, f_qj, Φ_q, K, h_z = cargar_parametros2()
...
objetivo = quicksum(x[z, T2] for z in Z)
...
model.addConstr((quicksum(x[z,T2] for z in Z) >= K),
                 name="r_9")
